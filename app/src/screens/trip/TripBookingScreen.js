...
// Returns next N upcoming trip dates using local date parts — avoids UTC shift
function getUpcomingTripDates(count = 6) {
  ...
  while (results.length < count && attempts < 60) {
    ...
    if (TRIP_DAYS.includes(dayName)) {
      // Build YYYY-MM-DD from local parts — never toISOString() which shifts to UTC
...
export default function TripBookingScreen({ navigation, route }) {
  ...
  // Auto-fill pickup house from employee profile
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        // silently fail — employee can enter manually
      } finally {
  ...
  // Load doctor directory for dropdown
  useEffect(() => {
    const loadDoctors = async () => {
      try {
        ...
        // silently fail — fall back to free text
      } finally {
  ...
  const validate = () => {
    ...
    if (seats < 1 || seats > MAX_SEATS) { alert(`Seats must be between 1 and ${MAX_SEATS}.`); return false; }
    return true;
  };
            ...
            <>
              ...
              >
                <Text style={styles.doctorDropdownText}>
                  {loadingDoctors ? 'Loading doctors...' : '🔍  Select from directory'}
                </Text>
              </TouchableOpacity>
              ...
              {showDoctorList && (
                <View style={styles.doctorList}>
                  {doctors.length === 0
                    ? <Text style={styles.doctorListEmpty}>No doctors in directory yet</Text>
                    : doctors.map(d => (
                      <TouchableOpacity key={d.id} style={styles.doctorListItem} onPress={() => selectDoctor(d)}>
...
const styles = StyleSheet.create({
  ...
  doctorList: {
    ...
    marginTop: 6, backgroundColor: '#ffffff', maxHeight: 200, overflow: 'hidden',
  },
