...
// Returns upcoming + today trip dates for filter tabs (Mon/Wed/Sat, next 4 weeks)
...
export default function TripReceptionHubScreen({ navigation, route }) {
  ...
  // Seat count — only confirmed bookings occupy a seat
  const confirmedCount = bookings.filter(b => b.status === 'confirmed').length;
  const pendingCount = bookings.filter(b => b.status === 'pending').length;
  const seatsLeft = SEAT_CAP - confirmedCount;

  // Report: only available after 16:00
  ...
  const handleGenerateReport = () => {
    if (!canGenerateReport()) {
      Alert.alert(
        'Too Early',
        'The trip report is available after 16:00. Please check back later.',
        [{ text: 'OK' }]
      );
      return;
    }
      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.employeeInfo}>
            ...
            <Text style={styles.employeeId}>{item.employeeNumber || '—'}</Text>
          </View>
