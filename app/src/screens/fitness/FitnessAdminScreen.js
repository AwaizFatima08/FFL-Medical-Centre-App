...
// Admin Incharge manages annual fitness appointments.
...
export default function FitnessAdminScreen({ navigation }) {
  ...
  const [reschedulePanel, setReschedulePanel] = useState(null); // { appointmentId, employeeName, currentDate, currentTime }
  const [newDate, setNewDate]             = useState('');
  const [newTime, setNewTime]             = useState('');

  // ── Reject reschedule panel ──
  const [rejectPanel, setRejectPanel]     = useState(null); // { appointmentId, employeeName }
  const [adminNote, setAdminNote]         = useState('');

  // ── Complete panel ──
  const [completePanel, setCompletePanel] = useState(null); // { appointmentId, employeeName }
  ...
  const handleSchedule = async () => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(schedDate)) {
      alert('Date must be in YYYY-MM-DD format. Example: 2025-06-15');
      return;
    }
    if (!/^\d{2}:\d{2}$/.test(schedTime)) {
      alert('Time must be in HH:MM format. Example: 09:30');
      return;
    }
      ...
      <View key={appt.id} style={[styles.card, needsRescheduleAction && styles.cardUrgent]}>
        ...
        {isActive && (
          <View style={styles.actions}>
            ...
            {/* Complete button — any active non-cancelled status */}
            {!needsRescheduleAction && (
              <TouchableOpacity
  ...
  const renderScheduleTab = () => (
    <View style={styles.scheduleForm}>
      ...
      <TouchableOpacity
        style={[styles.btnSchedule, scheduling && styles.btnDisabled]}
        ...
        <View style={styles.overlay}>
          <View style={styles.panel}>
            ...
            <Text style={styles.panelSubtitle}>
              For: {reschedulePanel.employeeName}
            </Text>
...
const styles = StyleSheet.create({
  ...
  cardUrgent: {
    borderWidth: 1.5, borderColor: '#f6ad55',
  },
  urgentBadge: {
  ...
  radioCircleSelected: {
    borderColor: '#3182ce', backgroundColor: '#3182ce',
  },
