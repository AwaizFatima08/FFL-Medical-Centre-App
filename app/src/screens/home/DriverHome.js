...
export default function DriverHome() {
  ...
  const getToken = async () => { const auth = getAuth(); return await auth.currentUser.getIdToken(); };

  const fetchActiveTrip = async () => {
    try {
      const token    = await getToken();
      const response = await fetch(`${API.ambulance}/driver/active`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleLogout = async () => {
    const confirmed = Platform.OS === 'web' ? window.confirm('Are you sure you want to logout?') : true;
    if (!confirmed) return;
    try { await signOut(auth); }
    catch (error) { Platform.OS === 'web' ? window.alert('Failed to logout.') : alert('Failed to logout.'); }
  };
  ...
  const handleCancel   = () => {
    const confirmed = Platform.OS === 'web' ? window.confirm('Cancel this trip? This cannot be undone.') : true;
    if (confirmed) callEndpoint('cancel', { reason: 'Cancelled by driver' });
  };
    ...
    return (
      <View style={styles.tripContainer}>
        ...
        {trip.priorityFlag === 'emergency' && (
          <View style={styles.emergencyBanner}><Text style={styles.emergencyText}>🚨 EMERGENCY</Text></View>
        )}
        <View style={styles.detailsCard}>
          ...
          <View style={styles.detailRow}>
            ...
            <View>
              ...
              <Text style={styles.detailValue}>{trip.vehicleAssigned === 'BLS' ? 'BLS Ambulance' : 'Mini Ambulance'}</Text>
            </View>
          </View>
          {trip.notes && (<><View style={styles.divider} /><View style={styles.detailRow}><Text style={styles.detailIcon}>📝</Text><View style={{ flex: 1 }}><Text style={styles.detailCaption}>Notes</Text><Text style={styles.detailValue}>{trip.notes}</Text></View></View></>)}
        </View>
        {actionLoading ? (
          <View style={styles.actionLoading}><ActivityIndicator size="large" color="#3182ce" /><Text style={styles.actionLoadingText}>Updating...</Text></View>
        ) : (
          <>
            {isDispatched && (<TouchableOpacity style={styles.arrivedBtn} onPress={handlePickedUp}><Text style={styles.arrivedBtnIcon}>📍</Text><Text style={styles.arrivedBtnText}>Arrived at Destination</Text></TouchableOpacity>)}
            {isPickedUp   && (<TouchableOpacity style={styles.returnedBtn} onPress={handleReturned}><Text style={styles.returnedBtnIcon}>🏥</Text><Text style={styles.returnedBtnText}>Back at Medical Centre</Text></TouchableOpacity>)}
            {canCancel    && (<TouchableOpacity style={styles.cancelBtn} onPress={handleCancel}><Text style={styles.cancelBtnText}>✕  Cancel Trip</Text></TouchableOpacity>)}
          </>
        )}
      </View>
    );
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {loading
          ? <View style={styles.centered}><ActivityIndicator size="large" color="#3182ce" /><Text style={styles.loadingText}>Loading trip...</Text></View>
...
const styles = StyleSheet.create({
  ...
  headerBar: { backgroundColor: '#2b6cb0', paddingTop: 48, paddingBottom: 16, paddingHorizontal: 20 },
  headerContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  headerTitle: { fontSize: 20, fontWeight: 'bold', color: '#ffffff' },
  headerSub: { fontSize: 13, color: '#bee3f8', marginTop: 2 },
  logoutBtn: { backgroundColor: '#e53e3e', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 6 },
  ...
  statusBanner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, borderRadius: 10, paddingVertical: 14 },
  statusIcon: { fontSize: 28 },
  statusLabel: { fontSize: 20, fontWeight: '800' },
  emergencyBanner: { backgroundColor: '#c53030', borderRadius: 8, paddingVertical: 10, alignItems: 'center' },
  emergencyText: { color: '#ffffff', fontWeight: '800', fontSize: 16, letterSpacing: 2 },
  detailsCard: { backgroundColor: '#ffffff', borderRadius: 12, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 4, elevation: 3 },
  ...
  arrivedBtn: { backgroundColor: '#276749', borderRadius: 12, paddingVertical: 18, alignItems: 'center', gap: 6 },
  arrivedBtnIcon: { fontSize: 32 },
  arrivedBtnText: { color: '#ffffff', fontSize: 16, fontWeight: '800' },
  returnedBtn: { backgroundColor: '#2b6cb0', borderRadius: 12, paddingVertical: 18, alignItems: 'center', gap: 6 },
  returnedBtnIcon: { fontSize: 32 },
  returnedBtnText: { color: '#ffffff', fontSize: 16, fontWeight: '800' },
  cancelBtn: { borderWidth: 1.5, borderColor: '#fc8181', borderRadius: 10, paddingVertical: 13, alignItems: 'center' },
