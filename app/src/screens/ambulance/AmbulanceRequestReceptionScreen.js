...
function SegmentedControl({ options, selected, onSelect }) {
  return (
    <View style={seg.row}>
      {options.map((opt) => (
        <TouchableOpacity key={opt.value} style={[seg.btn, selected === opt.value && seg.btnActive]} onPress={() => onSelect(opt.value)}>
...
function ReadOnlyField({ label, value }) {
  return (
    <View style={styles.readOnlyWrapper}>
      ...
      <View style={styles.readOnlyBox}><Text style={styles.readOnlyText}>{value || '—'}</Text></View>
    </View>
  );
}
...
export default function AmbulanceRequestReceptionScreen({ navigation }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const snapshot = await getDocs(query(collection(db, 'employees'), where('officialEmployeeNumber', '==', trimmed)));
      if (snapshot.empty) {
    ...
    <View style={styles.outer}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
        ...
        <View style={styles.searchRow}>
          <TextInput style={[styles.input, styles.searchInput]} placeholder="Enter official employee number" value={empNumber} onChangeText={(v) => { setEmpNumber(v); setSearchError(''); }} autoCapitalize="characters" />
          <TouchableOpacity style={[styles.searchBtn, searching && styles.searchBtnDisabled]} onPress={handleSearch} disabled={searching}>
        ...
        {employeeDoc && (
          <>
            ...
            <TextInput style={styles.input} value={patientName} onChangeText={setPatientName} placeholder="Full name of patient" />
            <Text style={styles.fieldLabel}>Relation to Employee</Text>
            <TextInput style={styles.input} value={patientRelation} onChangeText={setPatientRelation} placeholder="e.g. Self, Spouse, Child" />
            <Text style={styles.fieldLabel}>Condition / Complaint *</Text>
            <TextInput style={[styles.input, styles.multiline]} value={patientCondition} onChangeText={setPatientCondition} placeholder="Briefly describe the condition" multiline numberOfLines={3} />
            ...
            {priorityFlag === 'emergency' && <Text style={styles.emergencyWarning}>⚠️ Emergency requests take immediate priority.</Text>}
            ...
            <TextInput style={styles.input} placeholder={employeeDoc.houseNumber ? `Default: House ${employeeDoc.houseNumber}` : 'e.g. House 42, Block C'} value={pickupLocation} onChangeText={setPickupLocation} />
            {employeeDoc.houseNumber && !pickupLocation && <Text style={styles.hintText}>Leave blank to use employee's registered house number.</Text>}
